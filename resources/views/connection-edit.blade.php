...
@section('title', $connection->fromStation->name . ' → ' . $connection->toStation->name . ' bewerken')

@section('content')
    <div class="mx-auto max-w-md px-4 py-12">
        <a href="{{ route('connection.detail', $connection) }}" class="text-sm text-ink-muted hover:text-ink">← Terug naar verbinding</a>

        <h1 class="mt-4 text-2xl font-semibold text-ink">{{ $connection->fromStation->name }} → {{ $connection->toStation->name }} bewerken</h1>

        <form method="POST" action="{{ route('connection.update', $connection) }}" class="mt-8 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="from_station_id" name="from_station_id"
                    ...
                    @foreach ($stations as $station)
                        <option value="{{ $station->id }}" @selected(old('from_station_id', $connection->from_station_id) == $station->id)>
                            {{ $station->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="to_station_id" name="to_station_id"
                    ...
                    <option value="">Kies een stad</option>
                    @foreach ($stations as $station)
                        <option value="{{ $station->id }}" @selected(old('to_station_id', $connection->to_station_id) == $station->id)>
                            {{ $station->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input id="distance_km" type="number" name="distance_km" value="{{ old('distance_km', $connection->distance_km) }}"
                       class="mt-1.5 block w-full rounded-lg border border-line-strong bg-surface px-3 py-2">
                @error('distance_km')
                <p class="mt-1.5 text-sm text-danger">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input id="duration_minutes" type="number" name="duration_minutes" value="{{ old('duration_minutes', $connection->duration_minutes) }}"
                       class="mt-1.5 block w-full rounded-lg border border-line-strong bg-surface px-3 py-2">
