...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-12">
        ...
        <h1 class="mt-4 text-2xl font-semibold text-ink">Stad toevoegen</h1>

        <form method="POST" action="{{ route('station.store') }}" class="mt-8 space-y-5">
            @csrf
            ...
            <div>
                ...
                <input id="population" type="number" name="population" value="{{ old('population') }}"
                       class="mt-1.5 block w-full rounded-lg border border-line-strong bg-surface px-3 py-2">
                @error('population')
                <p class="mt-1.5 text-sm text-danger">{{ $message }}</p>
                @enderror
            ...
            <x-button full>Stad opslaan</x-button>
        </form>
    </div>
@endsection
